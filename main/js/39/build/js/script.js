...
function getCookie(nameCookie) {
  ...
  var cookieList = document.cookie.split('; ').map(function (cookieString) {
...
function setCookie(nameCookie, value, options) {
  ...
  var cookie = "".concat(nameCookie, "=").concat(value);
  if (options) {
...
var counter = document.querySelector('.title span');
var counterValue = getCookie(counterCookieName);
var newValue = 0;
...
function addUser(formId, userListSelector) {
  ...
  var userListElement = document.querySelector(userListSelector);
  var userList = [];
  var key = 'userList';
  var userId = 0;
  var actinSelector = '.user-action';
  var localStorageUserList = getFromLocalStorage(key);
  if (localStorageUserList) {
    localStorageUserList.forEach(function (user) {
  ...
  userListElement.addEventListener('click', function (e) {
    e.preventDefault();
    var tragetElement = e.target.closest(actinSelector);
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (this.classList.contains('user-edit')) {
      ...
      var oldUserList = userList;
      userList = userList.map(function (userOnList) {
...
function rerenderUserList(userList, oldUserList, userListElement) {
  var userListLength = userList.length - 1;
  for (var i = userListLength; i >= 0; i--) {
    if (userList[i] !== oldUserList[i]) {
      userListElement.children[userListLength - i].replaceWith(creatDomElementFromString(getUserTamplate(userList[i])));
    }
  }
}
function removeUser(userId, userList) {
  var userIndex = userList.map(function (user) {
    return user.id;
  }).indexOf(userId);
...
function editUser(userId, userList, form) {
  var user = getUserById(userId, userList);
  for (var key in user) {
    if (key === 'id') continue;
...
function renderUser(userListElement, user) {
  var li = getUserTamplate(user);
  userListElement.insertAdjacentHTML('afterbegin', li);
}
function getUserTamplate(user) {
  return "\n        <li>\n            User Name: ".concat(user['user-name'], "; \n            Email: ").concat(user.email, ";\n            Age: ").concat(user.age, ".\n            <button class=\"user-action remove\" data-user-id=\"").concat(user.id, "\">Remove User</button>\n            <button class=\"user-action edit\" data-user-id=\"").concat(user.id, "\">Edit User</button>\n        </li>\n    ");
}
