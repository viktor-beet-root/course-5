...
function getCarInfo() {
  var info = 'brand';
  for (var _info in carInfo) {
    console.log(_info, carInfo[_info]);
  }
...
function getFuelAndTimeAmountNeeded(distance) {
  var travelTimeWithoutBreaks = distance / carInfo.avaregeSpeed;
  var numberOfBreaks = Math.floor(travelTimeWithoutBreaks / 4);
  var breaksDuration = numberOfBreaks * 1;
  var totalTimeWithBreaks = travelTimeWithoutBreaks + breaksDuration;
  var fuelAmountNeeded = distance / 100 * carInfo.avaregeFuelCons;
...
var currentTime = createTime(12, 23, 45);
